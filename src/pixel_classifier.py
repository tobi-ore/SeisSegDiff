...
def compute_iou_original(args, preds, gts, print_per_class_ious=True):
...
def compute_iou(args, preds, gts, print_per_class_ious=True):
    ...
    for pred, gt in zip(preds, gts):
        for target_num in ids:
            ...
            unions[target_num] += (preds_tmp | gts_tmp).sum()
            intersections[target_num] += (preds_tmp & gts_tmp).sum()
            true_positives[target_num] += (preds_tmp & gts_tmp).sum()
            true_negatives[target_num] += ((1 - preds_tmp) & (1 - gts_tmp)).sum()
            false_positives[target_num] += (preds_tmp & (1 - gts_tmp)).sum()
            false_negatives[target_num] += ((1 - preds_tmp) & gts_tmp).sum()
            #class_frequencies[target_num] += gts_tmp.sum()
    ...
    recalls = []
    f1_scores = []
    #total_weight = sum(1 / freq for freq in class_frequencies.values())
    
    for target_num in ids:
        if target_num == args['ignore_label']: 
            continue
        iou = intersections[target_num] / (1e-8 + unions[target_num])
        accuracy = (true_positives[target_num] + true_negatives[target_num]) / (true_positives[target_num] + true_negatives[target_num] + false_positives[target_num] + false_negatives[target_num])
        precision = true_positives[target_num] / (true_positives[target_num] + false_positives[target_num] + 1e-8)
        recall = true_positives[target_num] / (true_positives[target_num] + false_negatives[target_num] + 1e-8)
        f1 = 2 * (precision * recall) / (precision + recall + 1e-8)
        #fwiu = (1/class_frequencies[target_num]) * iou / total_weight
        ious.append(iou)
        ...
        f1_scores.append(f1)
        if print_per_class_ious:
            print(f"IOU for {class_names[target_num]}: {iou:.4}, Class Accuracy: {accuracy:.4}, Precision: {precision:.4}, Recall: {recall:.4}, F1: {f1:.4}")

    mean_iou = np.array(ious).mean()
    mean_accuracy = np.array(accuracies).mean()
    #mean_fwiu = np.array(fwius).sum()
    mean_precision = np.array(precisions).mean()
    mean_recall = np.array(recalls).mean()
    mean_f1 = np.array(f1_scores).mean()
    
    print(f"Mean IOU: {mean_iou:.4}, Mean Accuracy: {mean_accuracy:.4}, Mean Precision: {mean_precision:.4}, Mean Recall: {mean_recall:.4}, Mean F1: {mean_f1:.4}")
    return mean_iou
