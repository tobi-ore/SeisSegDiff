...
def collect_features(args, activations: List[torch.Tensor], sample_idx=0):
    ...
    #save activations
    os.makedirs(os.path.join(args['exp_dir'], 'activations'), exist_ok=True)
    blocks = args['blocks']
    steps = args['steps']
    
    blks = blocks*len(steps)
    stps = [s for s in steps for _ in range(len(blocks))]

    i=0
    for act in activations:
    	#np.save(os.path.join(args['exp_dir'], 'activations', 'block_'+ str(blks[i]) + '_step_'+ str(stps[i]) +'.npy'), act.cpu().numpy())
    	i+=1
